...
Use [docs/visual_annotation/README.md](docs/visual_annotation/README.md) for package
docs, [tests/README.md](tests/README.md) for test-tree layout, and
[docs/visual_annotation/verification/README.md](docs/visual_annotation/verification/README.md)
...
```bash
uv run pytest tests/visual_annotation
```

Run only hermetic tests:

```bash
uv run pytest tests/visual_annotation -m hermetic
```
...
```bash
direnv exec . uv run python examples/visual_annotation/<module>.py
```

Keep examples focused on imports from `visual_annotation`. If an example
...
```bash
direnv exec . uv run python -m workbench.visual_annotation.<module>
```
...
direnv exec . uv run py-lib-reproduce-running-loop \
    workbench.visual_annotation.<module>
```
